...
func main() {
	...
	extractor := chatservice.NewClaudeExtractor(cfg.ClaudeAPIKey, cfg.ClaudeModel, logger)
	chUC := chatuc.New(client, extractor, logger)
	...
	addr := ":" + cfg.Port
	logger.Info("starting server", slog.String("addr", addr), slog.String("model", cfg.ClaudeModel))
