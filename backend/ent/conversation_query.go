...
type ConversationQuery struct {
	...
	withProposals *EventProposalQuery
...
func (_q *ConversationQuery) QueryMessages() *MessageQuery {
...
func (_q *ConversationQuery) QueryProposals() *EventProposalQuery {
...
func (_q *ConversationQuery) WithMessages(opts ...func(*MessageQuery)) *ConversationQuery {
	query := (&MessageClient{config: _q.config}).Query()
...
func (_q *ConversationQuery) WithProposals(opts ...func(*EventProposalQuery)) *ConversationQuery {
	query := (&EventProposalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ConversationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Conversation, error) {
	...
	if query := _q.withProposals; query != nil {
		if err := _q.loadProposals(ctx, query, nodes,
			func(n *Conversation) { n.Edges.Proposals = []*EventProposal{} },
			func(n *Conversation, e *EventProposal) { n.Edges.Proposals = append(n.Edges.Proposals, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ConversationQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Conversation, init func(*Conversation), assign func(*Conversation, *Message)) error {
...
func (_q *ConversationQuery) loadProposals(ctx context.Context, query *EventProposalQuery, nodes []*Conversation, init func(*Conversation), assign func(*Conversation, *EventProposal)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Conversation)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.EventProposal(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(conversation.ProposalsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.conversation_proposals
		if fk == nil {
			return fmt.Errorf(`foreign-key "conversation_proposals" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "conversation_proposals" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
