...
func (_q *MessageQuery) QueryConversation() *ConversationQuery {
...
func (_q *MessageQuery) QueryProposals() *EventProposalQuery {
...
func (_q *MessageQuery) WithConversation(opts ...func(*ConversationQuery)) *MessageQuery {
	query := (&ConversationClient{config: _q.config}).Query()
...
func (_q *MessageQuery) WithProposals(opts ...func(*EventProposalQuery)) *MessageQuery {
	query := (&EventProposalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := _q.withProposals; query != nil {
		if err := _q.loadProposals(ctx, query, nodes,
			func(n *Message) { n.Edges.Proposals = []*EventProposal{} },
			func(n *Message, e *EventProposal) { n.Edges.Proposals = append(n.Edges.Proposals, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MessageQuery) loadConversation(ctx context.Context, query *ConversationQuery, nodes []*Message, init func(*Message), assign func(*Message, *Conversation)) error {
...
func (_q *MessageQuery) loadProposals(ctx context.Context, query *EventProposalQuery, nodes []*Message, init func(*Message), assign func(*Message, *EventProposal)) error {
	...
	query.Where(predicate.EventProposal(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(message.ProposalsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.message_proposals
		if fk == nil {
			return fmt.Errorf(`foreign-key "message_proposals" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "message_proposals" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
